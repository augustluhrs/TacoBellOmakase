...
class TDA(ControlSurface):
	...
	def processSpecialMsgs(self):
		while self.specialMsgs:
			id, msgInfo = self.specialMsgs.popitem()
			for client in self.clients.values():
				self.send(client, msgInfo[1], msgInfo[0])

	def addSpecialMsg(self, id, address, msg):
		"""
		add a special msg to the queue

		args:
			id: a unique identifier. Each one will be run only once per update. 
				Often a pointer will be used
			address: OSC address
			msg: msg to send
		"""
		self.specialMsgs[id] = (address, msg)
...
#		self.song().view.selected_track = selected
	...
	def getTrackOrChainInfo(self, chain, index, name, expression):
		...
		try:
			chain.arrangement_clips
		except:
			chainInfo['aClips'] = []
		else:
			chainInfo['aClips'] = self.getAClips(chain)
	...
	def getAClips(self, chain):
		aClips = []
		for cIndex, aClip in enumerate(chain.arrangement_clips):
			aClipInfo = {}
			aClips.append(aClipInfo)
			aClipInfo['name'] = aClip.name
			aClipInfo['index'] = cIndex
			aClipInfo['expression'] = 'arrangement_clip[' +str(cIndex)+ ']'
			aClipInfo['startTime'] = aClip.start_time
			aClipInfo['endTime'] = aClip.end_time
			aClipInfo['startMarker'] = aClip.start_marker
			aClipInfo['endMarker'] = aClip.end_marker,
			aClipInfo['loopStart'] = aClip.loop_start,
			aClipInfo['loopEnd'] = aClip.loop_end,			
			if aClip.is_midi_clip:
				aClipInfo['filePath'] = ''
			else:
				aClipInfo['filePath'] = aClip.file_path
			aClipInfo['color'] = aClip.color
			aClipInfo['ptr'] = aClip._live_ptr
		return aClips
	...
	def startSongListeners(self):
		...
		for track in self.song().tracks:
			...
			try:
				...
				self.setupArrangementClipsListeners(track)
	...
	def setupArrangementClipsListeners(self, chain):
		def arrangementClipsChanged():			
			self.updateAClips(chain)
		chain.add_arrangement_clips_listener(arrangementClipsChanged)
		self.updateAClips(chain, sendUpdate=False)
	...
	def updateAClips(self, chain, sendUpdate=True):
		msg = []
		for index, aClip in enumerate(chain.arrangement_clips):
			if aClip._live_ptr not in self.watchedObjects:
				context = {'self': self, 'aClip': aClip}
				exec(	"def aClipChanged():\n"
					 	"	self.aClipChanged(aClip)\n"
						"def aClipPlaying():\n"
						"	self.aClipPlayingChange(aClip)\n"
						"aClip.add_name_listener(aClipChanged)\n"
						"aClip.add_color_listener(aClipChanged)\n"
						"aClip.add_end_time_listener(aClipChanged)\n"
						"aClip.add_start_marker_listener(aClipChanged)\n"
						"aClip.add_end_marker_listener(aClipChanged)\n"
						"aClip.add_loop_start_listener(aClipChanged)\n"
						"aClip.add_loop_end_listener(aClipChanged)\n"
						"aClip.add_playing_status_listener(aClipPlaying)\n"
						"if not aClip.is_midi_clip:\n"
						"	aClip.add_file_path_listener(aClipChanged)\n"
						"self.watchedObjects.add(aClip._live_ptr)"
						, context)
			if sendUpdate:
				msg.append([f'#',
							aClip.name,
							...
							aClip.loop_start,
							aClip.loop_end,
							'' if aClip.is_midi_clip else aClip.file_path,
							aClip.color,
							aClip._live_ptr])
		if sendUpdate:
			self.addSpecialMsg(chain._live_ptr, '/info/updateArrangementClips', 
								(chain._live_ptr, msg))

	def aClipPlayingChange(self, clip):
		self.addSpecialMsg(f'play{clip._live_ptr}', '/info/playingAClip',
							[clip.is_playing])
	...
	def aClipChanged(self, aClip):
		for client in self.clients.values():
			self.addSpecialMsg(aClip._live_ptr, '/info/aClipChanged',
						 [ptrStr(aClip._live_ptr),
								aClip.name,
								'' if aClip.is_midi_clip else aClip.file_path,
								aClip.color, aClip.start_time, aClip.end_time])
		if 	aClip._live_ptr not in self.watchedObjects:
				context = {'self': self, 'aClip': aClip}
				exec("def aClipChanged():\n"
					 "	self.aClipChanged(aClip)\n"
